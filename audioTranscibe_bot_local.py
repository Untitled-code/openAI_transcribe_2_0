...
import logging
import subprocess
import transcribe_openai_local
import pymysql
import os
import glob
import configparser
...
def transcribe(message):
    ...
    bot.send_message(chat_id, f'Закиньте сюди аудіо файл розміром до 20 Мб.'
                              f'Якщо файл більший, то завантажте на Dropbox, і надайте лінк (для юзерів 2-го рівня)')
    ...
    def audio(message):
        try:
            ...
            bot.send_message(chat_id, f'Щось пішло не так. Спробуйте пізніше. '
                                      ...
                                      f'\nВ цьому разі Вам потрібно підключити Dropbox.')
...
def finalRes(chat_id, filename, directory, TIMESTAMP):
    ...
    try:
        ...
        subprocess.call(['bash', 'audioConvert_local.sh', filename, newFile, str(chat_id)])
    except Exception as e:
        print(f"something wrong with the conversion. {e}")
        logging.info(f"something wrong with the conversion. {e}")
        bot.send_message(chat_id, f'Щось пішло не так. Спробуйте пізніше')
    try:
        transcribe_openai_local.main(newFile, directory, TIMESTAMP)
    except Exception as e:
        print(f"something wrong with the conversion. {e}")
        logging.info(f"something wrong with the conversion. {e}")
        bot.send_message(chat_id, f'Щось пішло не так. Спробуйте пізніше')
    output_file = f'./{directory}/audio_{TIMESTAMP}.txt'
    output_file_srt = f'./{directory}/audio_{TIMESTAMP}.srt'
    file = open(output_file, 'rb')
    file_srt = open(output_file_srt, 'rb')
    ...
    bot.send_document(chat_id, file_srt)  # sending file to user
